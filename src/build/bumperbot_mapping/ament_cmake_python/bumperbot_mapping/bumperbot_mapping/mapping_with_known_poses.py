...
#pega uma coordenada do tipo cartesiana (x,y)(dada em relação ao odom)e calcula qual seria suas coordenadas no mapa em si, no sistema do mapa 
...
#função que veriica se a coordenada esta no mapa ou nao e retorna um bool
...
#converte coordenadas (x, y) em um unico index, usado no vetor que armazena o grid 
...
class MappingWithKnownPoses(Node):
    def __init__(self, name):
        super().__init__(name)#nome do nó

        self.declare_parameter("width", 50.0)#em metros
        ...
        self.map_.info.width = round(width/resolution )#em numero de celulas
    ...
    def scanCallback(self, scan: LaserScan):
        ...
        if not poseOnMap(robot_p,  self.map_.info):
            self.get_logger().error("robot out of the map")
            return

        (raw, pitch, yaw) = euler_from_quaternion([t.transform.rotation.x,t.transform.rotation.y,t.transform.rotation.z,t.transform.rotation.w])

        for i in range(len(scan.ranges)):
            if math.isinf(scan.ranges[i]):
                continue

            angle = scan.angle_min + (i * scan.angle_increment) + yaw#angulo em relação ao odom 
