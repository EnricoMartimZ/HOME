...
class TurtleSpawner(Node):
    def __init__(self):
        super().__init__("turtle_spawner")#nome do nó
    ...
    def call_spawn_server(self, turtle_name, x, y, theta):
        ...
        while not client.wait_for_service(1.0):
            self.get_logger().info("Esperando servidor Spawn")
    ...
    def callback_call_spawn(self, future, turtle_name, x, y, theta):
        try:
            ...
            if response.name != "":
                self.get_logger().info("Tartaruga " + response.name + " vive")
                new_turtle = Turtle()
    ...
    def call_kill_server(self, turtle_name):
        ...
        while not client.wait_for_service(1.0):
            self.get_logger().info("Esperando servidor Kill")
