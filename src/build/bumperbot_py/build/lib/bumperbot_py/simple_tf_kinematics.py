...
class SimpleTfKinematics(Node):
    def __init__(self):
        ...
        self.orientation_increment_ = quaternion_from_euler(0, 0, 0.05) #radianos
        ...
        self.get_logger().info("Publishing static transfor between %s and %s" %
    ...
    def getTransformCallback(self, req, res):
        self.get_logger().info("Request Tranform between %s and %s" % (req.frame_id, req.child_frame_id))
        requested_tranform = TransformStamped()
        try:
            requested_tranform = self.tf_buffer_.lookup_transform(req.frame_id, req.child_frame_id, rclpy.time.Time())
        except TransformException as e:
            self.get_logger().error("an error occurred while transforming %s and %s" % (req.frame_id, req.child_frame_id))
            return res
        res.transform = requested_tranform
        res.success = True
        return res
