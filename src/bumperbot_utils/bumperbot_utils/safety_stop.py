...
class MyNode(Node):
    def __init__(self):
        ...
        while not self.decrease_speed_client_.wait_for_server(timeout_sec=1.0) and rclpy.ok():
            self.get_logger().info("Action /joy_turbo_decrease not available, waiting...")
        while not self.increase_speed_client_.wait_for_server(timeout_sec=1.0) and rclpy.ok():
            self.get_logger().info("Action /joy_turbo_increase not available, waiting...")
    ...
    def laserCallback(self, msg: LaserScan):
        ...
        if self.state_ != self.previous_state:
            ...
            self.previous_state = self.state_
            self.safety_stop_pub_.publish(is_safety_stop)
        else:

            if self.state_ == State.DANGER:
                is_safety_stop.data = True
            else:
                is_safety_stop.data = False
        ...
        if self.is_first_msg:
            for zone in self.zones.markers:
                #publicamos os marcadores com relação ao laser
